...
import { Loader2, Pencil, PlusCircle, SettingsIcon } from "lucide-react";
...
export default function ChaptersForm({ initialData, courseId }: ChaptersFormProps) {
  ...
  return (
    <div className="relative mt-6 border bg-slate-100 rounded-md p-4">
            ...
            <FormField
              ...
              render={({ field }) => (
                <FormItem>
                  ...
                  <FormControl>
                    <Input disabled={isSubmitting} placeholder="Enter the Chapter" {...field} />
                  </FormControl>
                  <FormDescription>Enter the Chapter</FormDescription>
                  <FormMessage />
                </FormItem>
              )}
            />
      ...
      {!isCreating && <p className="text-xs text-muted-foreground mt-4">Drag and drop to reorder the chapter</p>}
    </div>
  );
}
