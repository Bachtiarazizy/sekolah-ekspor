...
const VideoPlayer = ({ url, isLocked, nextChapterId, chapterId, courseId, completeOnEnd }: VideoPlayerProps) => {
  ...
  const onEnd = async () => {
    try {
      if (completeOnEnd) {
        const promise = axios.put(`/api/courses/${courseId}/chapters/${chapterId}/progress`, {
          isCompleted: true,
        });
    ...
    <div className="relative h-full w-full flex items-center justify-center !aspect-video">
      ...
      {!isLocked && (
        <ReactPlayer
          ...
          width={"100%"}
          className={"absolute inset-0 overflow-hidden !aspect-video rounded-2xl border !w-full !h-auto max-h-none"}
